...
public class CapacitorTagManagerPlugin: CAPPlugin {
    ...
    private var implementation: CapacitorTagManager?
    
    override public func load() {
        implementation = CapacitorTagManager()
    }
    ...
    @objc func getAppInstanceId(_ call: CAPPluginCall) {
        ...
        @objc func setConsent(_ call: CAPPluginCall) {
            guard let consentType = CapacitorTagManagerHelper.mapStringToConsentType(call.getString("type")) else {
                call.reject(errorConsentTypeMissing)
                return
            }
            guard let consentStatus = CapacitorTagManagerHelper.mapStringToConsentStatus(call.getString("status")) else {
